...
class BlenderRenderer:
    ...
    def __init__(self, blender_exec: str, config: Dict = None):
        """
        This BlenderRenderer class is the main interface for rendering rat's vision using Blender.
    ...
    def config_description() -> None:
        ...
        for key, value in BlenderRenderer.CONFIG_DESCRIPTION.items():
            print(f"\t{key}: {value}")
        BlenderRenderer._print_config_message()

    def print_config(self) -> None:
        """
        Prints the current configuration of the BlenderRenderer.
        """
    ...
    def update_config(self, config: Dict) -> None:
        """
        Updates the BlenderRenderer configuration with a new configuration dictionary.
    ...
    def _run_blender_command(
        ...
        cmd = [
            ...
            str(n_frames),
            "--render-output",
            os.path.join(self.config["output_dir"], f"frame{pad}.png"),
            "--render-anim",
            "--log-level",
            "0",
            "--",
    ...
    def render(
        self, positions: List[Tuple[float, float]], head_directions: List[float]
    ) -> None:
        """
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if self.config["env_file"] is None:
                    print(
                        "[*] no environment file provided, using default environment.\n"
                    )
                    blender_env_dir = files("ratvision.environments")
                    self.config["env_file"] = blender_env_dir.joinpath(
                        "box_messy.blend"
                    )
    ...
    def get_rendered_frames(self) -> List[Image.Image]:
        """
        Opens the rendered video in the default video player.

        Returns:
            List[Image.Image]: A list of PIL Image objects representing the rendered frames.
        """
        ...
        if not os.path.exists(output_dir):
            ...
            print(
                'you first need to render the rat\'s vision, or specify the correct "output_dir" in the config.'
            )
            return None
        ...
        return [Image.open(ff) for ff in frame_files]
