...
from ratvision import RaycastingRenderer, TorchRenderer
...
def main(benchmark):
    ...
    try:
        anim = RaycastingRenderer(config=config).get_video_animation(torch_frames.cpu())
    ...
    print(f'[NumPy]  {n_iter} frames in {np_elapsed:.4f}s  ({n_iter / np_elapsed:.0f} fps)')
    ...
    for i in range(0, n_iter, batch_size):
        torch_frames = torch_renderer(
            ...
            thetas[i:i+batch_size]
        )
        size = torch_frames.numel() * torch_frames.element_size() *1e-6 # MB
    _sync_kernels(device)
    torch_elapsed = time.perf_counter() - start
    print(f'[Torch]  {n_iter} frames with batch {batch_size} in {torch_elapsed:.4f}s  ({n_iter / torch_elapsed:.0f} fps)')
    print(f'         shape={tuple(torch_frames.shape)}, dtype={torch_frames.dtype}, memory={size:.1f} MB')
...
if __name__ == '__main__':
    ...
    argparser.add_argument(
        '--benchmark', type=argparse.BooleanOptionalAction, default=False,
        help='''
